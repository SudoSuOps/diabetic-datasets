...
def gen(q):
    ...
    with torch.no_grad(): o=model.generate(**ids,max_new_tokens=256,do_sample=False,pad_token_id=tok.pad_token_id)
    raw=tok.decode(o[0][ids['input_ids'].shape[1]:],skip_special_tokens=True)
    raw=re.sub(r"<think>.*?</think>","",raw,flags=re.DOTALL)
    raw=re.sub(r"^\s*(thinking process|reasoning)\b.*?(?:\n\s*\n|answer\s*:)","",raw,flags=re.DOTALL|re.IGNORECASE)
    return raw.strip()
...
FLUFF=("hello","wonderful","great question","happy to","i'm here to help","let's dive","absolutely","i'd love")
def composite(resp,ref):
    low=resp.lower(); wc=len(resp.split())
    g_esc=any(w in low for w in ESC)
    g_direct=not any(low.lstrip().startswith(f) or low[:60].find(f)>=0 for f in FLUFF)
    g_concise=wc<=90
    g_topic=len(words(resp)&words(ref))>=2 and wc>8
    return sum([g_esc,g_direct,g_concise,g_topic]),(g_esc,g_direct,g_concise,g_topic)
...
for i,r in enumerate(rows):
    ...
    t=gen(q)
    bs,_=composite(b,ref); ts,_=composite(t,ref)
    b_spec+=bs; t_spec+=ts; b_win+= bs>ts; t_win+= ts>bs
    if i<3: samples.append((q,ref,b,t,bs,ts))
n=len(rows)
log(f"avg PRODUCT-COMPOSITE /4 (esc+direct+concise+grounded)  BASE {b_spec/n:.2f}  vs  TUNED {t_spec/n:.2f}")
